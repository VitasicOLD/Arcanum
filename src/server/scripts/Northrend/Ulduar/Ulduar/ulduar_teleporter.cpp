...
public:
    ...
    {
        pPlayer->Dismount();
        pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Главный лагерь экспедиции", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + BASE_CAMP);
        if (InstanceScript* pInstance = pGO->GetInstanceScript())
        {
            if (pInstance->GetData(TYPE_COLOSSUS) == 2) //count of 2 collossus death
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Железный двор", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + GROUNDS);
            if (pInstance->GetBossState(TYPE_LEVIATHAN) == DONE)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Гигантская кузня", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + FORGE);
            if (pInstance->GetBossState(TYPE_XT002) == DONE)
            {
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Мусорная свалка", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + SCRAPYARD);
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Вестибюль Ульдуара", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + ANTECHAMBER);
            }
            if (pInstance->GetBossState(TYPE_KOLOGARN) == DONE)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Обвалившаяся галерея", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + WALKWAY);
            if (pInstance->GetBossState(TYPE_AURIAYA) == DONE)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Оранжерея Жизни", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + CONSERVATORY);
            if (pInstance->GetBossState(TYPE_FREYA) == DONE)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Искра воображения", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + SPARK);
            if (pInstance->GetBossState(TYPE_VEZAX) == DONE)
                pPlayer->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Провал Безумия", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + MADNESS);
        }
