...
class pack_of_vaccine : public ItemScript
{
public:
    pack_of_vaccine() : ItemScript("pack_of_vaccine") { }

    bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
    {
        if (player->GetQuestStatus(QUEST_WTWTR) == QUEST_STATUS_INCOMPLETE)
        {
            if (Creature* elk = player->FindNearestCreature(NPC_ELK, 20.0f, true))
            {
                player->KilledMonsterCredit(NPC_ELK, 0);
            }

            if (Creature* grizzly = player->FindNearestCreature(NPC_GRIZZLY, 20.0f, true))
            {
                player->KilledMonsterCredit(NPC_GRIZZLY, 0);
            }
        }
        return false;
    }
