...
class item_gt_kit : public ItemScript
{
    public:
        item_gt_kit() : ItemScript("item_gt_kit") { }

        bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
        {
            if (player->GetQuestStatus(QUEST_GLYPH_CHASING) == QUEST_STATUS_INCOMPLETE)
            {
                if (GameObject* regal = player->FindNearestGameObject(GO_REGAL, 5.0f))
                {
                    player->AddItem(ITEM_REGAL, 1);
                }

                if (GameObject* ash = player->FindNearestGameObject(GO_ASH, 5.0f))
                {
                    player->AddItem(ITEM_ASH, 1);
                }

                if (GameObject* zor = player->FindNearestGameObject(GO_ZOR, 5.0f))
                {
                    player->AddItem(ITEM_ZOR, 1);
                }
            }
            return false;
        }
