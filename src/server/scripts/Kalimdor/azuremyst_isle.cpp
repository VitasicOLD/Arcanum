...
class item_ravager_cage_key : public ItemScript
{
    public:
        item_ravager_cage_key() : ItemScript("item_ravager_cage_key") { }

        bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
        {
            if (Creature* ravager = player->FindNearestCreature(NPC_DEATH_RAVAGER, 5.0f, true))
            {
...
class item_bristlelimb_key : public ItemScript
{
    public:
        item_bristlelimb_key() : ItemScript("item_bristlelimb_key") { }

        bool OnUse(Player* player, Item* /*item*/, SpellCastTargets const& /*targets*/)
        {
            if (player->GetQuestStatus(QUEST_THE_PROPHECY_OF_AKIDA) == QUEST_STATUS_INCOMPLETE)
            {
                if (Creature* capitive = player->FindNearestCreature(NPC_STILLPINE_CAPITIVE, 5.0f, true))
                {
                    player->KilledMonsterCredit(NPC_STILLPINE_CAPITIVE, 0);
                }
            }
            return false;
        }
...
void AddSC_azuremyst_isle()
{
    ...
    new item_ravager_cage_key();
    new npc_stillpine_capitive();
    new go_bristlelimb_cage();
    new item_bristlelimb_key();
}
