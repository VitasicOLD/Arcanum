...
public:
    ...
    {
        if (player->GetQuestStatus(QUEST_DARKSTONE_OF_TERRROK) == QUEST_STATUS_INCOMPLETE)
        {
            if (Creature* capitive = player->FindNearestCreature(NPC_TERROKAR_QUEST_TARGET, 5.0f, true))
                player->CompleteQuest(QUEST_DARKSTONE_OF_TERRROK);
        }
        return false;
    }
};
