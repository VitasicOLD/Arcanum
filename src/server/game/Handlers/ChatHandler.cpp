...
void WorldSession::HandleMessagechatOpcode(WorldPacket & recv_data)
{
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_YELL:
        {
            ...
            if ((sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ)) && !sender->isGameMaster())
            {
                SendNotification(GetTrinityString(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_SAY_TIME_REQ)/60);
                return;
            }
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if ((sender->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ)) && !sender->isGameMaster())
            {
                SendNotification(GetTrinityString(LANG_CHANNEL_TIME_REQ), sWorld->getIntConfig(CONFIG_CHAT_WHISPER_TIME_REQ)/60);
                return;
            }
        ...
        case CHAT_MSG_CHANNEL:
        {
            if (AccountMgr::IsPlayerAccount(GetSecurity()))
            {
                ...
                if (_player->GetTotalPlayedTime() < sWorld->getIntConfig(CONFIG_CHAT_CHANNEL_TIME_REQ) && !_player->isGameMaster())
                {
