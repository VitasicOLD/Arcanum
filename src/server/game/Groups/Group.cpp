...
bool Group::IsGuildGroup(uint32 guildId, bool AllInSameMap, bool AllInSameInstanceId)
{
    ...
    bool ret = false;
    count = members.size();
    for (std::vector<Player*>::iterator itr = members.begin(); itr != members.end(); ++itr)
    {
            ...
            if (player->GetMap()->IsRaid() && !ret)
            {
                switch (player->GetMap()->GetDifficulty())
                {
                    ...
                    case RAID_DIFFICULTY_10MAN_HEROIC:
                        if (count >= sWorld->getIntConfig(CONFIG_BOSS_KILL_HISTORY_GUILD_GROUP_10_COUNT))
                            ret = true;
                        break;

                    case RAID_DIFFICULTY_25MAN_NORMAL:
                    case RAID_DIFFICULTY_25MAN_HEROIC:
                        if (count >= sWorld->getIntConfig(CONFIG_BOSS_KILL_HISTORY_GUILD_GROUP_25_COUNT))
                            ret = true;
                        break;
                }
            }
